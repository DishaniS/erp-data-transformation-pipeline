...
| file | purpose |
|---|---|
| `scripts/evaluate_phase4_identity_retrieval.py` | mini-evaluation |
| `docs/phase4_identity_aware_retrieval.md` | this report |
...
`scripts/evaluate_phase4_identity_retrieval.py` — 9 representations across 3
...
Artifact: `artifacts/phase4_identity_retrieval_evaluation.json`.
...
| artifact | status |
|---|---|
| `phase12_storage_benchmark.json` | unchanged (Aug 14) — `git diff` empty |
| `phase14_response_adaptation_evaluation.json` | unchanged (Aug 22) |
| `phase3_multimodal_evaluation.json` | re-run to confirm Phase 3 gates still hold; values identical |
| `phase13_openapi.json` | regenerated by its own test, as on every run; contains no filter-field enumeration, so the API schema is structurally unchanged |
| `phase4_identity_retrieval_evaluation.json` | **new** |
...
*See also: [Phase 3 — Database BLOB Multimodal Pipeline](phase3_database_blob_multimodal_pipeline.md),
[Phase 2 — Generic ERP Entity Support](phase2_generic_erp_entity_support.md).*
