...
| `artifacts/phase13_openapi.json` | Regenerated (by the existing test that owns it — see §8) |
...
**Purely additive.** No operation added or removed, no field removed. Verified by diffing `artifacts/phase13_openapi.json` against its committed version:
...
**Artifact regeneration:** `artifacts/phase13_openapi.json` is regenerated by an existing test (`tests/erp_pipeline/api/test_document_and_live_http.py:358`) as part of the normal suite — that is this project's convention, so no manual step was taken. The previous version is preserved in Git history at the consolidation commit.
