...
| File | Change |
|---|---|
| `tests/erp_pipeline/api/test_phase1_contract_correctness.py` | **NEW** — 41 tests covering fixes 1–7 |
...
`artifacts/phase13_openapi.json` is regenerated from the live app by
...
| File | Tests | Covers |
|---|---:|---|
| `tests/erp_pipeline/api/test_phase1_contract_correctness.py` | 41 | fixes 1–7, including empty / 1-row / 2-row / over-limit CSV, TSV, PDF, PNG, JPEG, identical-vs-different content, mixed-page OCR, corrupt / fake / empty / valid PDF, and the 500-still-works guard |
...
```
tests/erp_pipeline/api/test_phase1_contract_correctness.py
...
`artifacts/phase12_storage_benchmark.json` is unaffected — Phase 1 touched no
...
tests/erp_pipeline/api/test_phase1_contract_correctness.py       (new)
...
docs/phase1_contract_correctness_stabilization.md                (this report)
```

`artifacts/phase13_openapi.json` is regenerated by an existing contract test and
